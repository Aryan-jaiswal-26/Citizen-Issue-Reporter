...
const IssueCard = ({ item, onPress }: { item: Issue; onPress: () => void }) => (
  <TouchableOpacity style={styles.issueCard} onPress={onPress}>
...
export default function MyIssuesScreen() {
  ...
  const userIssues = mockIssues.filter((_, index) => index < 3); // Simulate user's issues
    ...
    <View style={styles.container}>
      ...
      {userIssues.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <IssueCard item={item} onPress={() => handleIssuePress(item)} />
          )}
